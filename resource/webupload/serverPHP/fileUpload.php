...
class FileUpload{
    ...
    //要配置的内容
    private $path = '../files';
    private $allowtype = array('jpg', 'gif', 'png', 'mp3');
    ...
    private function copyFile(){
        if (!$this->errorNum) {
            $day=date("d",time());
            $path_day=$this->path()."/".$day;
            if (!file_exists($path_day) || !is_writable($path_day) ){
                mkdir($path_day,"0777");
            }
            $path = rtrim($path_day, '/').'/';
...
if(isset($_POST['status'])){
    if($_POST['status'] == 'chunkCheck'){
        $target = '../files/'.$_POST['name'].'/'.$_POST['chunkIndex'];
...
$uploadpath="files/".$year."/".$day."/";
if(($path = $uploader->upload('file', $_POST)) !== false){
    die('{"status":1,"uploadpath":"'.$uploadpath.'", "path": "'.$path.'"}');
}
die('{"status":0}');
